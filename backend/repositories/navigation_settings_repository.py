...
class NavigationSettingsRepository:
            ...
            cursor.executemany(
                """INSERT INTO feature_visibility_settings
                   ...
                   ON DUPLICATE KEY UPDATE
                     ...
                     icon=VALUES(icon),route_path=VALUES(route_path)""",
                [
                    (
                        role,
                        item["id"],
    ...
    def replace_visibility(self, role: str, updates: list[dict[str, Any]], updated_by: str) -> list[dict[str, Any]]:
        ...
        normalized = [item for item in updates if str(item.get("id")) in valid_ids]
        with self.connection.cursor() as cursor:
                ...
                if "sort_order" in item:
                    cursor.execute(
                        """UPDATE feature_visibility_settings
                           SET is_visible=%s,sort_order=%s,updated_by=%s,updated_at=CURRENT_TIMESTAMP
                           WHERE role=%s AND feature_id=%s""",
                        (1 if is_visible else 0, int(item["sort_order"]), updated_by, role, feature_id),
                    )
                else:
                    cursor.execute(
                        """UPDATE feature_visibility_settings
