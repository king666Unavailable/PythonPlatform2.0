...
<template>
  <div class="page-stack">
    ...
    <section v-if="importLoading || importToken" class="content-card import-preview"><div class="section-heading"><div><h3>批量导入预览</h3><p v-if="importLoading">正在解析 Excel…</p><p v-else>请核对导入数据，确认无误后再写入数据库。默认密码为学号。</p></div><button v-if="!importLoading" class="secondary-button" type="button" @click="importToken = ''; importRows = []; importErrors = []; importError = ''">取消预览</button></div><InlineMessage :message="importError" tone="error" /><div v-if="!importLoading"><div class="import-result-summary"><span>可导入 {{ importRows.length }} 条</span><span :class="{ 'import-error-count': importErrors.length }">错误 {{ importErrors.length }} 条</span></div><div class="import-table" v-if="importRows.length || importErrors.length"><div class="import-table-head"><span>行号</span><span>学号</span><span>姓名</span><span>性别</span><span>行政班</span><span>教学班</span><span>校验结果</span></div><div v-for="item in [...importRows, ...importErrors]" :key="String(item.row)" class="import-table-row"><span>{{ item.row }}</span><span>{{ item.username }}</span><span>{{ item.name }}</span><span>{{ item.gender_label || '—' }}</span><span>{{ item.administrative_class || '—' }}</span><span>{{ item.teaching_class || '—' }}</span><span :class="{ 'import-error-text': item.errors }">{{ importErrorText(item) }}</span></div></div><button v-if="importRows.length && !importErrors.length" type="button" :disabled="working" @click="confirmImport">{{ working ? '导入中…' : `确认导入 ${importRows.length} 个账号` }}</button></div></section>
